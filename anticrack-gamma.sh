...
gchq_ips=("25.0.0.0/8")
mi6_ips=("195.59.0.0/16") # Example, as actual may not be public; added for completeness
...
# Eritrea (er) - No public DNS servers found
# Mali (ml)
"217.64.99.25" "196.200.48.40" "154.118.190.94"
# Central African Republic (cf) - No public DNS servers found
# Burkina Faso (bf)
...
# Add for other countries if specific lists exist, else default
...
ip_urls[ms]="https://download.microsoft.com/download/7/1/D/71D86715-5596-4529-9B13-DA13A5DE5B63/ServiceTags_Public_20240108.json" # Update date as needed
ip_urls[am]="https://ip-ranges.amazonaws.com/ip-ranges.json"
ip_urls[me]="https://www.gstatic.com/ipranges/goog.json" # Placeholder, Meta uses various; use Google as example or find specific
ip_urls[ap]="https://www.apple.com/certificateauthority/AppleWWDRCAG3.cer" # No IP list; perhaps no
ip_urls[nf]="https://ip-ranges.amazonaws.com/ip-ranges.json" # Netflix on AWS
...
function flush_rules() {
  ...
  nft add chain ip anticrack input { type filter hook input priority -10 \; policy drop \; } # Changed to drop for stricter
...
function total_lockdown() {
  ...
  # Block TOR bridges if list available; placeholder
  # curl -s https://bridges.torproject.org/bridges > /tmp/bridges.txt
  # Parse and block
