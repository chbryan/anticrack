...
function install_dependencies() {
  apt update -y || { echo "apt update failed." >> $LOG_FILE; exit 1; }
  apt install -y curl gnupg lsb-release software-properties-common || { echo "Installation of prerequisites failed." >> $LOG_FILE; exit 1; }
  curl -s https://packagecloud.io/install/repositories/crowdsec/crowdsec/script.deb.sh | bash || { echo "Adding CrowdSec repo failed." >> $LOG_FILE; exit 1; }
  apt update -y
  apt install -y nftables wget netfilter-persistent dnsmasq fail2ban suricata curl jq tor snort crowdsec crowdsec-firewall-bouncer-nftables obfs4proxy ufw || { echo "Installation failed." >> $LOG_FILE; exit 1; }
}
...
# Obfuscated bridges
obfs_bridges=(
"obfs4 177.235.180.240:8668 3AC17E97D18ADF92E9662D00EDE3B8FCCA6A6D80 cert=/WbHxF7lntn60mpieMRQ17YF88WfezgIzU0U+vQ3GMyCQUesVWtRkowZStfTO9cv6FutPw iat-mode=0"
...
function backup_configs() {
  cp $HOSTS_FILE $HOSTS_BACKUP || echo "Failed to backup hosts file." >> $LOG_FILE
  nft list ruleset > $NFT_BACKUP || echo "Failed to backup nft rules." >> $LOG_FILE
}

# Restore
function restore_configs() {
  cp $HOSTS_BACKUP $HOSTS_FILE || echo "Failed to restore hosts file." >> $LOG_FILE
  nft -f $NFT_BACKUP || echo "Failed to restore nft rules." >> $LOG_FILE
  rm -f $DNSMASQ_CONF || echo "Failed to remove dnsmasq conf." >> $LOG_FILE
  systemctl restart dnsmasq || echo "Failed to restart dnsmasq." >> $LOG_FILE
}

# Flush
function flush_rules() {
  nft flush table ip anticrack || nft add table ip anticrack
  nft add chain ip anticrack input { type filter hook input priority -10 \; policy accept \; } # Changed to accept for allowing others
  nft add chain ip anticrack output { type filter hook output priority -10 \; policy accept \; }
  cp $HOSTS_BACKUP $HOSTS_FILE || echo "Failed to restore hosts." >> $LOG_FILE
  > $DNSMASQ_CONF
  systemctl restart dnsmasq || echo "Failed to restart dnsmasq." >> $LOG_FILE
}
...
# Enable TOR obfuscation
function enable_tor_obfs() {
  echo "UseBridges 1" >> /etc/tor/torrc
  echo "ClientTransportPlugin obfs4 exec /usr/bin/obfs4proxy" >> /etc/tor/torrc
  for bridge in "${obfs_bridges[@]}"; do
    echo "Bridge $bridge" >> /etc/tor/torrc
  done
  systemctl restart tor || echo "Failed to restart tor." >> $LOG_FILE
  echo "TOR with obfuscation enabled."
}

# Integrate UFW
function integrate_ufw() {
  ufw default deny incoming || echo "UFW default deny incoming failed." >> $LOG_FILE
  ufw default allow outgoing || echo "UFW default allow outgoing failed." >> $LOG_FILE
  ufw allow 22/tcp || echo "UFW allow SSH failed." >> $LOG_FILE
  echo y | ufw enable || echo "UFW enable failed." >> $LOG_FILE
  echo "UFW integrated with basic rules. Note: May conflict with nft rules if not configured properly."
}

# Setup CrowdSec
function setup_crowdsec() {
  systemctl enable --now crowdsec || echo "CrowdSec enable failed." >> $LOG_FILE
  cscli collections install crowdsecurity/linux || echo "CrowdSec collection install failed." >> $LOG_FILE
  cscli parsers install crowdsecurity/whitelists || echo "CrowdSec parser install failed." >> $LOG_FILE
  systemctl reload crowdsec || echo "CrowdSec reload failed." >> $LOG_FILE
  systemctl enable --now crowdsec-firewall-bouncer || echo "CrowdSec bouncer enable failed." >> $LOG_FILE
...
function block_entity() {
  ...
  curl -s $d_url | eval $d_proc >> /tmp/domains.txt || echo "Failed to fetch domains for $code." >> $LOG_FILE
  ...
  if [[ -n $i_url ]]; then
    curl -s $i_url | eval $i_proc > /tmp/ips.txt || echo "Failed to fetch IPs for $code." >> $LOG_FILE
  elif [[ $code == "nsa" ]]; then
    printf "%s\n" "${nsa_ips[@]}" > /tmp/ips.txt
  elif [[ $code == "fbi" ]]; then
    printf "%s\n" "${fbi_ips[@]}" > /tmp/ips.txt
  ...
  apt purge -y $pkgs 2>/dev/null || echo "Failed to purge packages for $code." >> $LOG_FILE
...
function disable_entity() {
  local code=$1
  sed -i "/# anticrack-start $code/,/# anticrack-end $code/d" $HOSTS_FILE || echo "Failed to remove hosts entries for $code." >> $LOG_FILE
  sed -i "/$code/d" $DNSMASQ_CONF || echo "Failed to remove dnsmasq entries for $code." >> $LOG_FILE
...
function apply_all() {
  ...
  systemctl restart dnsmasq || echo "Failed to restart dnsmasq." >> $LOG_FILE
  netfilter-persistent save || echo "Failed to save netfilter." >> $LOG_FILE
}
...
function total_lockdown() {
  block_group "all"
  # Block TOR exits
  curl -s https://www.dan.me.uk/torlist/ > /tmp/tor.txt || echo "Failed to fetch TOR exits." >> $LOG_FILE
  while read -r ip; do
    nft add rule ip anticrack input ip saddr $ip drop
    nft add rule ip anticrack output ip daddr $ip drop
  done < /tmp/tor.txt
  ...
  systemctl restart fail2ban suricata snort || echo "Failed to restart services." >> $LOG_FILE
  suricata-update || echo "Suricata update failed." >> $LOG_FILE
  # Rate limiting SSH
...
function menu() {
  while true; do
    ...
    echo "10) Enable TOR obfuscation"
    echo "11) Integrate UFW"
    echo "12) Exit"
