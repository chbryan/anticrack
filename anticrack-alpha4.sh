...
apt update -y
apt install -y ipset wget iptables-persistent
...
function enable_country() {
  ...
  ip6tables -A INPUT -m set --match-set "block-$c" src -j DROP
  ip6tables -A OUTPUT -m set --match-set "block-$c" dst -j DROP
...
function disable_country() {
  ...
  ip6tables -D INPUT -m set --match-set "block-$c" src -j DROP 2>/dev/null
  ip6tables -D OUTPUT -m set --match-set "block-$c" dst -j DROP 2>/dev/null
...
function enable_dns_block() {
  ...
  ipset create block-dns hash:ip family inet
  for ip in "${dns_ips[@]}"; do
    ipset add block-dns "$ip" 2>/dev/null
  done
  ipset create block-dns6 hash:ip family inet6
  ...
  ip6tables -A INPUT -m set --match-set block-dns6 src -j DROP
  ip6tables -A OUTPUT -m set --match-set block-dns6 dst -j DROP
...
function disable_dns_block() {
  ...
  ip6tables -D INPUT -m set --match-set block-dns6 src -j DROP 2>/dev/null
  ip6tables -D OUTPUT -m set --match-set block-dns6 dst -j DROP 2>/dev/null
  ipset destroy block-dns 2>/dev/null
  ipset destroy block-dns6 2>/dev/null
...
function total_lockdown() {
  ...
  echo "Total lockdown enabled - all enemy blocks."
}
...
function block_domain() {
  local d=$1
  if [[ "$d" == *'*'* ]]; then
    echo "Wildcard not supported in hosts; block specific subdomains manually."
    return
  fi
  if [ "$(status_domain $d)" == "Blocked" ]; then
...
function list_blocked_domains() {
  grep "$BLOCK_MARKER" "$HOSTS_FILE" | awk '{print $2 " - Blocked."}' > "$BLOCKED_DOMAINS_FILE"
...
function enable_logging() {
  iptables -A INPUT -j LOG --log-prefix "AntiCrack DROP: "
  iptables -A OUTPUT -j LOG --log-prefix "AntiCrack DROP: "
  ip6tables -A INPUT -j LOG --log-prefix "AntiCrack DROP: "
  ip6tables -A OUTPUT -j LOG --log-prefix "AntiCrack DROP: "
  echo "Firewall logging enabled."
}

function disable_logging() {
  iptables -D INPUT -j LOG --log-prefix "AntiCrack DROP: " 2>/dev/null
  iptables -D OUTPUT -j LOG --log-prefix "AntiCrack DROP: " 2>/dev/null
  ip6tables -D INPUT -j LOG --log-prefix "AntiCrack DROP: " 2>/dev/null
  ip6tables -D OUTPUT -j LOG --log-prefix "AntiCrack DROP: " 2>/dev/null
...
list_countries
echo "Improved with IPv6 DNS, logging, wildcard note, auto-install, network down/up."

while true; do
  echo "1) List status - Shows all blocks."
  echo "2) Enable country - Block IP ranges."
  ...
  echo "8) Block domain - Enter domain (no wildcard; add subs manually)."
  echo "9) Unblock domain - Enter domain."
  ...
  echo "14) Enable domain group (tech/enemy_social/enemy_media/military) - Block group domains."
  ...
  echo "22) Total lockdown - All enemy blocks."
  echo "23) Exit"
  ...
  case $choice in
    ...
    21) restore_rules ;;
    22) total_lockdown ;;
    23) read -p "Save rules? (y/n): " save
       if [ "$save" == "y" ]; then iptables-save > /etc/iptables.rules; ip6tables-save > /etc/ip6tables.rules; echo "Saved."; fi
       exit 0 ;;
    *) echo "Invalid." ;;
  esac
done
