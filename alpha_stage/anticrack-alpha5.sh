...
function install_dependencies() {
  ...
  apt install -y ipset wget iptables-persistent || { echo "Installation failed."; exit 1; }
}
...
function enable_country() {
  ...
  check_command ipset create "block-$c" hash:net
  while read -r ip; do
  ...
  check_command ip6tables -A INPUT -m set --match-set "block-$c" src -j DROP
  check_command ip6tables -A OUTPUT -m set --match-set "block-$c" dst -j DROP
...
function disable_country() {
  ...
  ip6tables -D INPUT -m set --match-set "block-$c" src -j DROP 2>/dev/null || true
  ip6tables -D OUTPUT -m set --match-set "block-$c" dst -j DROP 2>/dev/null || true
...
function status_domain() {
  local d=$1
  grep -q "^0.0.0.0 $d $BLOCK_MARKER" "$HOSTS_FILE" && echo "Blocked" || echo "Not blocked"
}
...
function list_blocked_domains() {
  grep "$BLOCK_MARKER" "$HOSTS_FILE" | awk '{print $2 " - Blocked."}' > "$BLOCKED_DOMAINS_FILE" || { echo "Failed to list domains."; exit 1; }
...
list_countries
echo "Improved with robust error handling, auto-install."
